      ...
      <div class="row g-0">
          <div class="col-md-4">
              <div style="position: relative;">
                  <img src="{{ Storage::url($vehiculo->imagen) }}" class="img-fluid rounded-start w-100 h-100" style="object-fit: cover;" id="imagenCard">
              </div>
          </div>
          <div class="col-md-8">
              <div class="card-body">
...
<script>
  ...
  document.getElementById('formArriendo').addEventListener('submit', function(event) {
    ...
    const pathWithoutStorage = imagenSrc.replace(window.location.origin + '/storage/', '');
    document.getElementById('imagenInput').value = pathWithoutStorage;
});
